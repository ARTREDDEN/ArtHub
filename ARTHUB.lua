...
MainTab:AddButton("🚁 วาร์ปไปหาเฮลิคอปเตอร์", function()
...
MainTab:AddToggle("⛓️ เปิดระบบฟาร์มเงินอัตโนมัติ", function(state)
...
PlayerTab:AddToggle("🧱 เปิดระบบเดินทะลุกำแพง (Noclip)", function(state)
    Settings.Noclip = state
end)

PlayerTab:AddToggle("🦘 เปิดระบบกระโดดไม่จำกัด (Inf Jump)", function(state)
    Settings.InfJump = state
end)

PlayerTab:AddSlider("⚡ ความเร็วตัวละคร (WalkSpeed)", 16, 500, 16, function(value)
...
VisualTab:AddToggle("👁️ เปิดมองทะลุ (กรอบน้ำเงิน + ชื่อ)", function(state)
    Settings.ESP = state
end)

-- ==== Credits ====
local CreditTab = CreateTab("Credits")
CreditTab:AddButton("ART HUB - Fixed UI", function()
    print("สคริปต์ทำงานเสร็จสมบูรณ์เรียบร้อย!")
end)
